...
def ensure_rgb_for_jpeg(
    image: Image.Image, background: tuple = (255, 255, 255)
) -> Image.Image:
    """Return an RGB image that can be saved as JPEG.

    JPEG does not support alpha. RGBA/LA images (and palette images with
    transparency) are composited onto *background* first.
    """
    if image.mode == "RGB":
        return image
    ...
    if image.mode == "P" and "transparency" in image.info:
        return ensure_rgb_for_jpeg(image.convert("RGBA"), background)

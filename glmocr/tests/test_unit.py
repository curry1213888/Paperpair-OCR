...
class TestUtils:
    ...
    def test_ensure_rgb_for_jpeg_from_rgba(self, tmp_path):
        """RGBA crops can be saved as JPEG (regression for transparent PNG sources)."""
        from PIL import Image

        from glmocr.utils.image_utils import crop_image_region, ensure_rgb_for_jpeg

        rgba = Image.new("RGBA", (40, 40), (255, 0, 0, 128))
        cropped = crop_image_region(rgba, [0, 0, 1000, 1000])
        assert cropped.mode == "RGBA"

        out = tmp_path / "crop.jpg"
        ensure_rgb_for_jpeg(cropped).save(out, format="JPEG", quality=95)

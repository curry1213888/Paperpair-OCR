...
from glmocr.utils.markdown_utils import image_prefix_from_source
...
class Pipeline:
    ...
    def __init__(
        ...
        # LLM 审核器（可选，由环境变量控制；见 .env.example）
        try:
            from glmocr.postprocess.llm_reviewer import LLMReviewer

            self._reviewer = LLMReviewer.from_env()
            if self._reviewer is not None:
                logger.info(
                    "LLM 审核器已启用（model=%s, base_url=%s）",
                    self._reviewer.model,
                    self._reviewer.base_url,
                )
        except Exception as _reviewer_init_err:
            logger.warning("LLM 审核器加载失败，已跳过：%s", _reviewer_init_err)
            self._reviewer = None
    ...
    def _emit_results(
        ...
        while (
            ...
            img_prefix = image_prefix_from_source(original_inputs[u])
            ...
            result = PipelineResult(
                ...
                reviewed_raw_json_result=reviewed_raw_json,
                model_review_report=review_report,
            )
            built.add(u)
            if preserve_order:
                pending[u] = result

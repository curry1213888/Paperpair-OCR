...
from glmocr.utils.image_utils import ensure_rgb_for_jpeg
...
class BaseParserResult(ABC):
    ...
    def __init__(
        ...
        reviewed_raw_json_result: Optional[list] = None,
        model_review_report: Optional[dict] = None,
    ):
        """Initialize.
        ...
        Args:
            ...
            raw_json_result: Raw model output before post-processing.
                When LLM reviewer is disabled: saved as ``{name}_model.json``.
                When LLM reviewer is enabled: saved as ``{name}_model_raw.json``.
            reviewed_raw_json_result: LLM-corrected model output; saved as
                ``{name}_model.json`` when present.
            model_review_report: LLM review metadata (changes, confidence, latency);
                saved as ``{name}_model_review_report.json`` when present.
        """
        ...
        self.reviewed_raw_json_result = reviewed_raw_json_result
        self.model_review_report = model_review_report
    ...
    def _save_json_and_markdown(self, output_dir: Union[str, Path]) -> None:
        ...
        if self.raw_json_result is not None:
            # 若 LLM 审核器已启用（reviewed_raw_json_result 存在），
        ...
        # LLM 审核后的 model 输出（仅审核器启用时存在）
        if self.reviewed_raw_json_result is not None:
        ...
        # LLM 审核报告（改动清单、置信度、延迟等）
        if self.model_review_report is not None:
            report_file = output_path / f"{base_name}_model_review_report.json"
            try:
                with open(report_file, "w", encoding="utf-8") as f:
                    json.dump(self.model_review_report, f, ensure_ascii=False, indent=2)
            except Exception as e:
                logger.warning("Failed to save review report: %s", e)
        ...
        if self.image_files:
            ...
            for filename, img in self.image_files.items():
                try:
                    ensure_rgb_for_jpeg(img).save(
                        imgs_dir / filename, format="JPEG", quality=95
                    )
                except Exception as e:
